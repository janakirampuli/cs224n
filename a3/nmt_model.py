...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(in_channels=embed_size, out_channels=embed_size, kernel_size=2, stride=1, padding="same")
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        last_hidden = last_hidden.transpose(0, 1).reshape(b, -1)
        init_decoder_hidden = self.h_projection(last_hidden)
        # print(init_decoder_hidden.shape)

        last_cell = last_cell.transpose(0, 1).reshape(b, -1)
