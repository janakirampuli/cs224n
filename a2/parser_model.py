...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.rand((n_features * self.embed_size, hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(torch.rand(hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.dropout = nn.Dropout(self.dropout_prob)

        self.hidden_to_logits_weight = nn.Parameter(torch.rand((hidden_size, n_classes)))
        self.hidden_to_logits_bias = nn.Parameter(torch.rand(n_classes))
